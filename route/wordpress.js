...
var Author, Category, Post, Tag, areChildrenTextOnly, async, authorPosts, authors, baseUrl, categories, categoryPosts, cleanUpAttributes, config, datePosts, dates, filterEmptyChildren, fs, jsdom, mapChildNode, mapChildNodes, mergeSiblingTexts, post, processAuthorInformations, processLanguageInformations, processRequest, processTextElements, recentPosts, restructureChildren, stringifyChildren, tagPosts, tags, transformPost, transformPostContent, utils, _;
    ...
    return Post.count({}, function(error, count) {
      ...
      return Post.find({}).sort("-date").limit(limit).exec(function(err, posts) {
        if (err) {
          ...
          return async.map(posts, transformPost, function(err, posts) {
            return res.json({
              count: limit,
              pages: pages,
              total: total,
